 ...
 * @param sids
 */
pro.getUids = function(sids) {
  let result = [];
  let self = this;
  if (Array.isArray(sids)) {
    sids.forEach(function(sid){
      let session = self.session.service.get(sid);
      if (session && session.uid) {
        result.push(session.uid);
      }
    })
  }
  return result;
...
pro.send = function(reqId, route, msg, recvs, opts, cb) {
  let uids = this.getUids(recvs);
  logger.debug('[%s] send message uids: %j, reqId: %s, route: %s, msg: %j, receivers: %j, opts: %j', this.app.serverId, uids, reqId, route, msg, recvs, opts);
  var emsg = msg;
  if(this.encode) {
...
var getSession = function(self, socket) {
  ...
  logger.debug('[%s] getSession session is created with session id: %s ip:%s port:%s', app.getServerId(), sid, socket.remoteAddress.ip, socket.remoteAddress.port);
...
var handleMessage = function(self, session, msg) {
  logger.debug('[%s] handleMessage uid: %d, sid: %s, msg: %j', self.app.serverId, session.uid, session.id, msg);
